...
# the output folder
output_folder <- file.path("proteoform_detection", "output", "proteogenomics", "leiden")
...
# import PSMs
psms <- import_psms_from_nf(file = file.path("data", "proteogenomics", "novel_and_canonical_target_psmtable.txt"),
                            id_col = "Gene Name",
...
saveRDS(object = similarities, file = file.path(output_folder, "pg_similarities.RDS"))

similarities <- readRDS(file = file.path("proteoform_detection", "output", "pg_similarities.RDS"))
...
# store
saveRDS(object = graphs, file = file.path(output_folder, "pg_graphs.RDS"))
...
# store
saveRDS(object = graphs, file = file.path(output_folder, "pg_graphs_comms.RDS"))

# graphs <- readRDS(file = file.path(output_folder, "pg_graphs_comms.RDS"))
...
saveRDS(object = proteoforms, file = file.path(output_folder, "pg_proteoforms.RDS"))

# proteoforms <- readRDS(file = file.path(output_folder, "pg_proteoforms.RDS"))
...
graphs_to_plot <- graphs %>%
  ...
  .[lapply(., get.graph.attribute, "num_peptides_in_largest_community") > 10]
...
iois <- c("NCOA2", "NR3C1", "ABL1", "ZAP70", "TCF3", "IKZF1", "CDK16", "BCL9", "WDFY3", "TMPO", "OFD1", "GAPDH", "PKM", "KAT8")

export_to_viz(iois = iois,
              graphs = graphs,
              peptide_features = fData(peptides),
              membership_colors = membership_colors,
              identifier = "id",
              ensp_column = "first_protein_id",
              output_folder = "/Users/matthias.stahl/ki/network_medicine/pepnet/pepnetvis/public/data/")


es <- get_ioi_e_set(ioi = "PKM", e_set = peptides)
